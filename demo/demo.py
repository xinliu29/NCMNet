...
from ncmnet import NCMNet as Model   # Conference model
#from ncmnet_plus import NCMNet as Model    # Journal model
...
def demo(opt, img1_path, img2_path):
    ...
    model = Model(opt)
    checkpoint = torch.load('../model/yfcc/model_best.pth', map_location=torch.device('cpu'))   # Conference model
    #checkpoint = torch.load('../model/yfcc/model_best_ncmnet_plus.pth', map_location=torch.device('cpu'))   # Journal model
