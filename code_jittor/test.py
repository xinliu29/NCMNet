...
def dump_res(measure_list, res_path, eval_res, tag):
    ...
    #     # ofn = './log/test/median_err_q_ours.txt'
    ...
    with jittor.no_grad():
        for test_data in loader_iter:
            ...
            if config.use_fundamental:
                # unnorm F
                e_hat = jittor.matmul(jittor.matmul(test_data['T2s'].transpose(1,2), e_hat.reshape(-1,3,3)),test_data['T1s'])
                # get essential matrix from fundamental matrix
                e_hat = jittor.matmul(jittor.matmul(test_data['K2s'].transpose(1,2), e_hat.reshape(-1,3,3)),test_data['K1s']).reshape(-1,9)
...
def test(data_loader, model, config):
    ...
    try:
        ...
        # 加载模型
        pytorch_ckpt = torch.load(save_file_best,
                                map_location='cpu',
                                weights_only=False)  # 显式设置 weights_only

        # 转换为 Jittor 格式
        jittor_state_dict = {}
        for k, v in pytorch_ckpt['state_dict'].items():
            k = k.replace('module.', '')
            jittor_state_dict[k] = jittor.array(v.numpy())

        # 加载参数
        model.load_parameters(jittor_state_dict)
        # model.cu()
        ...
        raise e
    # Restore model
    # checkpoint = model.load(save_file_best)
    start_epoch = pytorch_ckpt['epoch']
