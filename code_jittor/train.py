...
def train_step(step, optimizer, model, match_loss, data):
    ...
    loss,_,_,_,_,_ = match_loss.run(step, data, logits, ys_ds, e_hat, y_hat)
    # geo_loss, cla_loss, l2_loss, _, _  = 0
    #loss += loss_i
...
def train(model, train_loader, valid_loader, config):
    ...
    if config.resume:
        ...
        checkpoint = jittor.load(checkpoint_path)
    ...
    for step in trange(start_epoch, config.train_iter, ncols=config.tqdm_width):
        ...
        # cur_lr = optimizer.lr
        train_step(step, optimizer, model, match_loss, train_data)
