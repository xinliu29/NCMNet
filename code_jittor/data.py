...
import cv2
import jittor as jt
from jittor.dataset import Dataset,VarDataset
...
def collate_fn(batch):
    ...
    for key in ['K1s', 'K2s', 'Rs', 'ts', 'xs', 'ys', 'T1s', 'T2s','virtPts']:
        data[key] = jt.array(np.stack(data[key])).float32()

    if data['sides'] != []:
        data['sides'] = jt.array(np.stack(data['sides'])).float32()
    return data
...
class CorrespondencesDataset(Dataset):
    def __init__(self, filename, config):
        ...
        self.data = None
        # self.set_attrs(total_len=None)
        # self.set_attrs(_disable_workers = False)
