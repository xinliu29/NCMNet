...
def sc(x):
    K1, K2 = x[:,:,:,:2].squeeze(1), x[:,:,:, 2:].squeeze(1)
    l1 = torch.norm((K1[:, :, None, :] - K1[:, None, :, :]), dim=-1)
    l2 = torch.norm((K2[:, :, None, :] - K2[:, None, :, :]), dim=-1)
    com =  l1 -l2 
    com = torch.clamp(1.0 - com ** 2 / 0.2 ** 2, min=0)  
...
class GCN_GN(nn.Module):
    def __init__(self, in_channel):
        super(GCN_GN, self).__init__()
        self.in_channel = in_channel
        self.conv = nn.Sequential(
    ...
    def graph_aggregation(self, x, w, sc):
        B, _, N, _ = x.size() 
        A_w = self.attention(w)
        #A = (1-self.gamma1) * A_w+ self.gamma1 * gc
        A = A_w *  sc
        with torch.no_grad():     
            #print(self.gamma1)
    ...
    def forward(self, x, w, sc):
        out = self.graph_aggregation(x, w, sc)
        out = self.conv(out)
        return out
...
class MH_Attention(nn.Module):
    def __init__(self, in_channel, heads = 4, dim_head = 32, dropout = 0.1):
        super().__init__()
    ...
    def forward(self, q, k, v):
        ...
        q1 = torch.split(q1, h_dim, 1)
        k1 = torch.split(k1, h_dim, 1)
        v1 = torch.split(v1, h_dim, 1)

        for i in range(h):
            scores = torch.bmm(q1[i].squeeze(3).transpose(1, 2), k1[i].squeeze(3))
            att = torch.softmax(scores, dim=2)

            if i == 0:
                out = torch.bmm(v1[i].squeeze(3), att.transpose(1, 2))
                result_att = out
            else:
                result_att_prior = result_att[:, h_dim * (i-1): h_dim * i, :]
                
                result_att_prior = torch.sigmoid(result_att_prior)
          
                v_new = v1[i].squeeze(3) * result_att_prior
                out = torch.bmm(v_new, att.transpose(1, 2))
                result_att = torch.cat((result_att, out), dim=1)
 
        out = result_att.unsqueeze(-1)
...
class CCI_Transformer_Layer(nn.Module):
    def __init__(self, in_channel):
        nn.Module.__init__(self)

        dropout = 0.1

        self.MH_att = MH_Attention(in_channel, heads = 4, dim_head = 32, dropout = 0.1)

    def forward(self, n1, n2, n3):
        
        out = self.MH_att(n1, n2, n3)
...
class DS_Block(nn.Module):
    def __init__(self, initial=False, predict=False, out_channel=128, k_num=8, sampling_rate=0.5):
        ...
        self.gcn1 = GCN_GN(self.out_channel)
        self.gcn2 = GCN_GN(self.out_channel)
        self.gcn3 = GCN(self.out_channel)
        ...
        self.cci1 = CCI_Transformer_Layer(self.out_channel)
        self.cci2 = CCI_Transformer_Layer(self.out_channel)
        self.cci3 = CCI_Transformer_Layer(self.out_channel)
        ...
        self.cci11 = CCI_Transformer_Layer(self.out_channel)
        self.cci22 = CCI_Transformer_Layer(self.out_channel)
        self.cci33 = CCI_Transformer_Layer(self.out_channel)
        ...
        self.down11 = diff_pool(self.out_channel, 250)
        self.l22 = []
        for _ in range(2):
            self.l22.append(OAFilter(self.out_channel, 250))
        self.up11 = diff_unpool(self.out_channel, 250)
        self.l22 = nn.Sequential(*self.l22)
    ...
    def forward(self, x, y):
        ...
        out_gs1 = self.gcn1(out, w_p1.detach(), sc_g)
        ...
        out_gs2 = self.gcn2(out, w_p2.detach(),sc_g)
...
class NCMNet(nn.Module):
    def __init__(self, config):
        ...
        self.ds_0 = DS_Block(initial=True, predict=False, out_channel=128, k_num=9, sampling_rate=0.5)
